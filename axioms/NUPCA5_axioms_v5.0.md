NUPCA5 AXIOMS — v5.01 (PASTABLE / PLAIN TEXT)
(= NUPCA3 AXIOMS 1.6.3 + scan-proof signature-addressed retrieval patch: A0.BUDGET + A4.3′ + A4.5 + A16.6′ + A7.3 + a minimal A13 ordering splice.)


==========================
5.01 changes to implement:
==========================

Sig64(t) MUST be computed only from a discriminative, collision-audited observed bundle F(t) = {small committed metadata + ephemeral peripheral gist} whose in-manifold Bayes ceiling for the chosen regime label satisfies best_acc(L|F) ≥ 0.97 (measured by test6.py), otherwise the encoding (not memory size) MUST be changed before scaling Stage-1 storage.
...
A0.BUDGET.1 Hard caps (configured or compile-time; no silent growth):
- N_MAX stored executed units
- K_MAX retrieved/engaged units per step
- C_CAND_MAX gated candidates after Stage-1
- PENDING_MAX deferred validations
- Fixed index constants: F_MAX (max selected blocks), L_TABLES, N_BUCKETS, BUCKET_CAP

A0.BUDGET.2 Two-stage retrieval is mandatory:
- Stage 1: cheap gate → bounded candidate set
- Stage 2: exact scoring on candidates only → Top-K (K ≤ K_MAX)

A0.BUDGET.3 No full scans:
- Stage 1 touches a constant number of fixed-capacity buckets/slots independent of N_MAX
- Stage 2 evaluates candidates only
- Any “compare against all” or “iterate all stored units” per step is forbidden

A0.BUDGET.4 Fixed per-step compute; deterministic degradation:
- Memory ops must fit in the same per-step budget B_rt
- If exceeded, degrade deterministically (reduce probed blocks, reduce tables, reduce K, skip inserts/maintenance), never spike

A0.BUDGET.5 Store at execution factorization:
- The stored/indexed/updated unit is exactly the executed unit that contributes to fusion

A0.BUDGET.6 Lean packed representations only:
- Archive/index/pending are packed arrays/structs addressed by integer IDs
- No per-unit Python dict/list object graphs as the durable “database”

A0.BUDGET.7 Outcome-vetted updates only:
- Retrieval/activation/fusion weight must not update retrieval stats or retention stats
- Only deferred external validation at the relevant horizon/mask may update them

A0.BUDGET.8 Discriminability-gated growth:
- New durable units (or durable versions) may be created only when an addressable key
  available at decision time (block id + signature bucket id) distinguishes the regime
- Otherwise treat residual as aliasing/uncertainty; do not allocate persistent structure
...
A4.3′ Signature-addressed cold storage retrieval (BLOCK-SCOPED; SCAN-PROOF; SUPERCEDES A4.3).
Definitions:
- Stored/indexed/updated object is an executed unit u (primitive operator contributing to fusion).
- Each unit stores: unit_block[u], unit_sig64[u] (64-bit address, immutable at creation),
  and outcome-vetted reliability stats err_ema[u,h_bin], val_count[u,h_bin]
  where h_bin ∈ {NEG,ZERO,POS} = sign(h).

Signature computation (strict scope):
- Compute sig64(t) within-step from:
  (i) committed observation METADATA from O_t (counts / small histograms only; no per-dim values),
  and (ii) an ephemeral peripheral gist p(t) available before purge.
- Forbidden: persisting p(t), any O_t per-dimension values, or any dense intermediate feature vectors
  in durable state or serialization; only fixed-width keys (sig64 and unit_sig64) and
  outcome-vetted scalars/counters may persist.

Stage 1 (gate; scan-proof):
- Index is fixed-capacity buckets: Bucket[b, j, bucket_id] → ≤ BUCKET_CAP unit_ids stored in packed arrays.
- For selected blocks B(t) (|B| ≤ F_MAX) and each table j ∈ [0..L_TABLES):
  - bucket_id = H_j(sig64(t)) (fixed bitselect/shift)
  - fetch that bucket; union+bounded dedup in a fixed probe order → Cand(t);
    once |Cand|=C_CAND_MAX, drop any additional candidates deterministically (no growth).
  - Fixed probe order is: blocks in the agent’s selected order; within block tables j ascending;
    within each bucket slots in ascending index.
- Ban: any iteration over all stored units.

Stage 2 (exact scoring; candidates only):
- score(u) = -popcount(sig64(t) XOR unit_sig64[u]) - α * err_ema[u,h_bin]
- return Top-K by score, K ≤ K_MAX.

Overflow/eviction (mandatory):
- Bucket insertion when full: replace lowest-priority occupant using a priority derived only
  from outcome-vetted stats (never usage-count).
- If N_MAX reached: evict lowest-priority units before creating new ones; if eviction prohibited
  by policy, refuse creation and log.
...
A4.5 Deferred outcome validation (SOLE SOURCE OF RETRIEVAL-STAT UPDATES; NEW).
- When unit u contributes at time t for horizon h ≥ 0, write a pending record into a fixed-capacity ring:
  (unit_id, t_due=t+h, h_bin, frame_id, footprint_id, pred_ref)

- At t_due, compute overlap set E in canonical state-index space:
  E = map_to_canonical(footprint(u), frame_id) ∩ O_{t_due},
  where frame_id MUST match the observation-frame mapping used to form O_{t_due}
  (including any transport shift).

  - If |E|=0: no update.
  - Else compute error on E and update:
      err_ema[u,h_bin] (negative evidence included; no “good-only” gate),
      val_count[u,h_bin]++

- Pending overflow: if full, evict oldest pending record and log.
- Forbidden updates: retrieval frequency, activation frequency, fusion weight, internal similarity.

Serialization rule:
- pending ring and pred_ref are NOT serialized; pred_ref is a bounded index into transient
  per-step prediction snapshots (values on footprint(u) only), never a durable object reference.
...
A7.3 Unified temporal completion operator (NEW).
- Complete(prior, cue, h, context) → estimate(t+h) with h ∈ ℤ.
- h=0 perception: clamp cue into prior.
- h>0 prediction: forward dynamics under the same model class.
- h<0 recall: inference-only by default (no durable episodic store introduced by this patch).
- Same model weights across h; h is an explicit conditioning input, not a mode switch.
- Training must include h=0 with weight ≥ any |h|>0 to anchor perception.
...
5.5) Retrieval address (agent-internal): compute sig64(t) per A4.3′ (metadata from O_t + p(t)),
     then perform scan-proof cold-storage retrieval per A4.3′ to make retrieved units eligible
     for A5 active-set selection within the same step budget.
...
- “expected error” is computed from the fused predictive distribution (mean + uncertainty), not just historical residual.
...
A16.6′ Cold storage retrieval coupling (SUPERCEDES A16.6).
- Block selection B(t) defines execution scope:
  only units whose footprints intersect selected blocks are retrieval-eligible
  (enforced structurally by the block-scoped index in A4.3′).
- Signature determines rank within scope (A4.3′ Stage 2); block scores do not define rank.
- Clamping by O_t remains authoritative; units may only affect priors within their footprints.
...
=========================================
Memory Contract (convenience restatement; normative = A0.BUDGET + A4.3′ + A4.5)
=========================================

Discriminability-gated growth:
- Create or version nodes/constellations only when a bounded-cost cue available at decision time
  can discriminate the new regime; otherwise treat residual error as uncertainty and do not allocate
  new persistent structure.

Persistent summaries are addresses, not data:
- They may bias retrieval, but they cannot be used to reconstruct, supervise, or replay sensory;
  only externally validated observation–prediction error updates memory.

Bounded growth:
- Node creation, edge creation, and any episodic/summary buffers have hard caps (compile-time constants
  or explicitly configured limits).
- If the cap is hit: evict / merge / refuse creation. No silent growth.

No full scans:
- Retrieval must be sublinear in total node count N: O(1) bucket lookup or O(log N) index lookup.
...
“Memory must be hard-capped, retrieval must be sublinear with two-stage Top-K gating, no full scans,
no unbounded composition search, no usage-based reinforcement, and all memory ops must fit a fixed
per-step budget using lean packed representations.”

Because observations arrive in real time, the agent must rapidly learn some tractable factorization (not “everything”), so the environment should be curated to ramp complexity where learning signal exists, while longer-horizon inference may run asynchronously in additional platform-dependent threads (≥2 sufficient for proof) without changing system dynamics—only improving learning rate.